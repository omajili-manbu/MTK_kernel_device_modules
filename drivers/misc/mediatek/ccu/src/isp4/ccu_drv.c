	...
	unsigned long arg)
{
	...
	case CCU_IOCTL_SET_POWER:
	{
		...
		LOG_DBG("[IOCTL_DBG] (void *)arg: %lx\n", (void *)arg);
		LOG_DBG("[IOCTL_DBG] ptr_power32: %lx\n", ptr_power32);
		LOG_DBG("[IOCTL_DBG] ptr_power64: %lx\n", ptr_power64);
...
static int ccu_alloc_command(struct ccu_cmd_s **rcmd)
{
	...
	if (cmd == NULL) {
		LOG_ERR("%s(), node=0x%llx\n", __func__, (uint64_t)cmd);
		return -ENOMEM;
	}
...
static void ccu_qos_init(struct ccu_device_s *ccu)
{
...
static void ccu_qos_uninit(struct ccu_device_s *ccu)
{
	mutex_lock(&ccu_qos_mutex);
#if (CCU_QOS_SUPPORT_ENABLE)
...
int ccu_clock_enable(void)
{
	...
	LOG_DBG_MUST("%s, %d clks, 2 pwr. %d\n", __func__, clock_num, _clk_count);
...
static long ccu_ioctl(struct file *flip, unsigned int cmd, unsigned long arg)
{
	...
	case CCU_IOCTL_GET_IOVA:
	{
		...
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_get fd=%d va=%llx\n", fd, (uint64_t)buf);
		...
		if (IS_ERR(ccu_iova[iova_buf_count].attach)) {
			LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_attach failed, attach=%llx dmabuf=%llx\n",
			 (uint64_t)ccu_iova[iova_buf_count].attach, (uint64_t)ccu_iova[iova_buf_count].dma_buf);
			goto err_attach;
		}
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_attach va=%llx dmabuf=%llx\n",
			(uint64_t)ccu_iova[iova_buf_count].attach, (uint64_t)ccu_iova[iova_buf_count].attach->dmabuf);
		...
		if (IS_ERR(ccu_iova[iova_buf_count].sgt)) {
			LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] map_attachment failed, attach=%llx sgt=%llx opt=%d\n",
			 (uint64_t)ccu_iova[iova_buf_count].attach, (uint64_t)ccu_iova[iova_buf_count].sgt,
			 DMA_BIDIRECTIONAL);
			goto err_map;
		}
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] dma_buf_map_attachment sgt=%llx\n",
		...
		iova_buf_count++;
		LOG_ERR(
			"[CCU_IOCTL_GET_IOVA] sg_dma_address, ret=%llx\n", (uint64_t)dma_addr);
...
static int ccu_probe(struct platform_device *pdev)
{
	...
	if ((strcmp("ccu", g_ccu_device->dev->of_node->name) == 0)) {
		...
		g_ccu_device->ccu_base =
			devm_ioremap(g_ccu_device->dev, phy_addr, phy_size);
		...
		LOG_DBG("of_parse_phandle(), smi_node=%lx\n", smi_node);
		smi_pdev = of_find_device_by_node(smi_node);
		if (WARN_ON(!smi_pdev)) {
			of_node_put(smi_node);
			return -ENODEV;
		}
		LOG_DBG("of_parse_phandle(), smi_pdev=%lx\n", smi_pdev);
		of_node_put(smi_node);
		LOG_DBG("of_node_put(%lx)\n", smi_node);
		...
		LOG_INF_MUST("probe 1, ccu_base:irq_num: %d, pdev: %llx\n",
...
// #endif
			...
			bit_mask_val = 31;
#endif
			if (dma_set_mask_and_coherent(g_ccu_device->dev,
				DMA_BIT_MASK(bit_mask_val))) {
				LOG_DERR(g_ccu_device->dev,
					"dma_set_mask_and_coherent(%u) return error.",
					bit_mask_val);
				goto EXIT;
			}
