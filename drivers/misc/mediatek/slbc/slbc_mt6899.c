...
static struct slbc_config p_config[] = {
	...
	SLBC_ENTRY(UID_MML,        0,   0,   0,   0, 0x0,   0x030, 0),
	SLBC_ENTRY(UID_DISP,       1,   0,   0,   0, 0x0,   0x030, 0),
	SLBC_ENTRY(UID_MM_VENC,    2,   0,   0,   0, 0x0,   0x3c0, 0),
	SLBC_ENTRY(UID_AINR,       3,   0,   0,   0, 0x0,   0x00c, 0),
	SLBC_ENTRY(UID_APU,        4,   0,   0,   0, 0x0,   0x003, 0),
};
...
static int slbc_shared_dram_init(struct platform_device *pdev)
{
	...
	if (slbc_mem_phys_addr == 0 || slbc_mem_virt_addr == 0 || slbc_mem_size == 0 ) {
		pr_info("[SLBC] phys_addr/virt_addr/mem_size of sspm shared dram init fail\n");
		return -1;
	}
...
static void slbc_remove(struct platform_device *pdev) /* rodin b4: 6.18 remove void */
{
	struct device *dev = &pdev->dev;
