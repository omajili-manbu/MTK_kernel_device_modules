...
#endif
#include "include/panel_o10_42_02_0a_alpha_data.h"
#include "include/panel-o10-42-02-0a-dsc-vdo.h"
...
static struct mtk_panel_params ext_params_30hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[3] = {0, 3, {0x5F, 0x00,0x40} },
#ifdef CONFIG_FACTORY_BUILD
		.dfps_cmd_table[4] = {0, 2, {0x6F,0x04} },
		.dfps_cmd_table[5] = {0, 3, {0x51,0x01,0x55} },
		.dfps_cmd_table[6] = {0, 1, {0x39} },
#else
		.dfps_cmd_table[4] = {0, 1, {0x39} },
#endif
	},
...
static struct mtk_panel_params ext_params_60hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[0] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x03} },
		.dfps_cmd_table[1] = {0, 2, {0x6F,0x03} },
		.dfps_cmd_table[2] = {0, 2, {0xBA,0x80} },
...
static void mi_parse_cmdline_perBL(struct LHBM_WHITEBUF * lhbm_whitebuf) {
	...
	pr_info("lhbm_1200 \n");
	for (i = 0; i < 6; i++){
		lhbm_normal_white_1200nit[18].para_list[i] = lhbm_whitebuf->lhbm_1200[i];
		pr_info("0x%02hhx",lhbm_whitebuf->lhbm_1200[i]);
	}

	pr_info("lhbm_200 \n");
	for (i = 0; i < 6; i++){
		lhbm_normal_white_200nit[18].para_list[i] = lhbm_whitebuf->lhbm_200[i];
...
static int mi_disp_panel_update_lhbm_code(struct mtk_dsi * dsi, enum lhbm_cmd_type type, int flat_mode, int bl_level)
{
	...
	case TYPE_WHITE_1200:
		if (flat_mode) {
			...
			lhbm_normal_white_1200nit[19].para_list[12] = alpha_buf[0];
			lhbm_normal_white_1200nit[19].para_list[13] = alpha_buf[1];
		} else {
	...
	case TYPE_WHITE_200:
		if (flat_mode) {
			...
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
					type, alpha_buf[0],  alpha_buf[1]);
			lhbm_normal_white_200nit[19].para_list[12] = alpha_buf[0];
			lhbm_normal_white_200nit[19].para_list[13] = alpha_buf[1];
		} else {
			alpha_buf[0] = (giroff_alpha_set[bl_level] >> 8) & 0xff;
			alpha_buf[1] = giroff_alpha_set[bl_level] & 0xff;
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
	...
	case TYPE_GREEN_500:
		if (flat_mode) {
			...
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
					type, alpha_buf[0],  alpha_buf[1]);
			lhbm_normal_green_500nit[19].para_list[12] = alpha_buf[0];
			lhbm_normal_green_500nit[19].para_list[13] = alpha_buf[1];
		} else {
			alpha_buf[0] = (giroff_alpha_set[bl_level] >> 8) & 0xff;
			alpha_buf[1] = giroff_alpha_set[bl_level] & 0xff;
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT_RESTORE:
		...
		if (atomic_read(&doze_enable)) {
			if (mi_cfg->last_bl_level) {
				...
				mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, flat_mode, 0);
				mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off), format);
			}
		}else{
			mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, flat_mode, bl_level);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s o10-42-02-0a-dsc-vdo -\n", __func__);
	return ret;
}
...
MODULE_DESCRIPTION("o10_42_02_0a_dsc_vdo oled panel driver");
