/* rodin stage2: replaced by the 6.18 upstream implementation (same lineage).
 * The 6.6 vendor copy used key->raw / int-returning program_key / an
 * ops->program_key member that no longer exist in 6.18 struct definitions.
 * No vendor host driver relies on the dropped hook. */
