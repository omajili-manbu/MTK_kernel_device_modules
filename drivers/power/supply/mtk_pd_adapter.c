...
struct mtk_pd_adapter_info {
	...
	int *pd_type;
	struct ta_info_notifier_block *pd_nb;
	bool force_cv;
				...
				unsigned long event, void *data)
{
	...
	struct ta_info_notifier_block *pd_nb =
		container_of(nb, struct ta_info_notifier_block, nb);
	...
	case TCP_NOTIFY_PD_STATE:
		...
		ret = usb_set_property(USB_PROP_PD_TYPE, info->pd_type[idx]);
    ...
    case TCP_NOTIFY_CVDM:
            pr_info("%s: tcpc received cvdm message.\n", __func__);
            ret = srcu_notifier_call_chain(&adapter->evt_nh,
                    MTK_PD_CVDM, &noti->cvdm_msg);
            break;
...
static int pd_get_svid(struct adapter_device *dev)
{
	...
	pr_info("%s: enter\n", __func__);
	if (info->adapter->adapter_svid != 0 && info->adapter->adapter_svid != 0xff00)
		return MTK_ADAPTER_OK;
...
{
	int i;
	struct tcp_dpm_custom_vdm_data cvdm_data = event->tcp_dpm_data.cvdm_data;

	pr_info("%s: cvdm_data.cnt = %d\n", __func__, cvdm_data.cnt);
	for (i = 0; i < cvdm_data.cnt; i++)
		pr_info("%s cvdm_data.data[%d] = 0x%08x", __func__, i,
			cvdm_data.data[i]);
	return 0;
}
				 ...
				 int active_idx, struct adapter_power_cap *cap)
{
	...
	if (pd_cap.nr != 0) {
		...
		for (i = 0; i < pd_cap.nr; i++) {
			...
			if (cap->maxwatt[i] > apdo_max && cap->type[i] == 3)
		...
		usb_set_property(USB_PROP_ADAPTER_IMAX, adapter_imax);
		usb_set_property(USB_PROP_ADAPTER_NR, pd_cap.nr);
	}
...
static void mtk_pd_adapter_remove(struct platform_device *pdev) /* rodin b4: 6.18 remove void */
{
