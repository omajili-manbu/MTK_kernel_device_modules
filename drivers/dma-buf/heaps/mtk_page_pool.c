...
int mtk_dmabuf_page_pool_init_shrinker(void)
{
	...
	shrinker = shrinker_alloc(0, "mtk-dmabuf-page-pool");
	if (!shrinker)
		return -ENOMEM;
	shrinker->count_objects = mtk_dmabuf_page_pool_shrink_count;
	shrinker->scan_objects = mtk_dmabuf_page_pool_shrink_scan;
	shrinker->seeks = 16;
	shrinker->batch = 0;
