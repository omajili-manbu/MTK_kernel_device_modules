...
static __init int emicen_init(void)
{
	...
	if (ret) {
		pr_info("emicen: failed to register driver\n");
		return ret;
	}
	...
	ret = emiisu_init();
	if (ret) {
		pr_info("emicen: failed to init isu\n");
		return ret;
	}
