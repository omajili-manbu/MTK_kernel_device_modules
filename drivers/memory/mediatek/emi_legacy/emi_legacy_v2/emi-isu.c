...
static int emiisu_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_info(&pdev->dev, "No buf_size\n");
		return -EINVAL;
	}

	ret = of_property_read_u64(emiisu_node, "buf_addr", &addr_temp);
	if (ret) {
		dev_info(&pdev->dev, "No buf_addr\n");
		return -EINVAL;
	}
	if (addr_temp)
	...
	if (ret) {
		dev_info(&pdev->dev, "Failed to create buf file\n");
		return ret;
	}
...
static void emiisu_remove(struct platform_device *pdev) /* rodin stage2: 6.18 .remove is void */{
...
int emiisu_init(void)
{
	...
	if (ret) {
		pr_info("emiisu: failed to register dirver\n");
		return ret;
	}
	...
	if (ret) {
		pr_info("emiisu: failed to create control file\n");
		return ret;
	}
